/*
 * Description: Computer vision api from OPENCV;
 * can use to implement own image recognition.
 */
...
public class CVActivity extends Activity implements CvCameraViewListener {
    ...
    public void setCameraDisplayOrientation(CVActivity activity,
                                            int cameraId, JavaCameraView camera) {
        android.hardware.Camera.CameraInfo info =
                new android.hardware.Camera.CameraInfo();
        android.hardware.Camera.getCameraInfo(cameraId, info);
        int rotation = ((Activity) mOpenCvCameraView0.getContext()).getWindowManager().getDefaultDisplay().getRotation();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mOpenCvCameraView0 = (JavaCameraView) findViewById(R.id.javaView);

        if (iNumberOfCameras > 1)
            mOpenCvCameraView1 = (JavaCameraView) findViewById(R.id.javaView2);

        setCameraDisplayOrientation(this, Camera.CameraInfo.CAMERA_FACING_BACK, mOpenCvCameraView0);
        ...
        if (iNumberOfCameras > 1) {
            ...
            mOpenCvCameraView1.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT));
        }
        ...
        if (item.getItemId() == R.id.action_info) {
            ...
            if (iNumberOfCameras > 1) {
                if (iCamera == 0) {
                    ...
                    mOpenCvCameraView1 = (JavaCameraView) findViewById(R.id.javaView);
                    mOpenCvCameraView1.setCvCameraViewListener(this);
                    ...
                    mOpenCvCameraView1.setVisibility(SurfaceView.GONE);
                    mOpenCvCameraView0 = (JavaCameraView) findViewById(R.id.javaView2);
                    mOpenCvCameraView0.setCvCameraViewListener(this);
                    mOpenCvCameraView0.setVisibility(SurfaceView.VISIBLE);
    ...
    private void ShowTitle (String s, int iLineNum, Scalar color) {
        Core.putText(mRgba, s, new Point(10, (int)(dTextScaleFactor * 20 * iLineNum)),
                Core.FONT_HERSHEY_COMPLEX_SMALL, dTextScaleFactor, color, 2);
    }
